...
MEMSIZE="$[10 * (1 << 20)]"
CGDIR=/sys/fs/cgroup/memory/mycgroup
mkdir -p $CGDIR
...
page-types -p $pid -a 0x700000000+100

cat $CGDIR/cgroup.procs
echo $MEMSIZE, $[MEMSIZE / 2], $[MEMSIZE * 3 / 2]

# assuming cgroup2 is mounted.
echo "----- pid:${pid}"
echo $[MEMSIZE / 2]     > $CGDIR/memory.limit_in_bytes
echo $[MEMSIZE * 3 / 2] > $CGDIR/memory.memsw.limit_in_bytes

cat $CGDIR/memory.usage_in_bytes
cat $CGDIR/memory.limit_in_bytes
cat $CGDIR/memory.memsw.usage_in_bytes
cat $CGDIR/memory.memsw.limit_in_bytes

kill -SIGUSR1 $pid
echo "dadadsda" /dev/kmsg
...
echo $[1 << 50] > $CGDIR/memory.memsw.limit_in_bytes
echo $[1 << 50] > $CGDIR/memory.limit_in_bytes
