...
check_and_define_tp memeater_random
prepare_multiple_injection_race_tmp() {
...
cleanup_multiple_injection_race_tmp() {
    set_monarch_timeout $DEFAULT_MONARCH_TIMEOUT
    save_nr_corrupted_inject
    all_unpoison
	ipcrm --all
...
__control_multiple_injection_race_process() {
	local pid=$1

	$PAGETYPES -b $TARGET_PAGEFLAG -p $pid -rNl | grep -v X | grep -v offset > $TMPF.page-types
	local nr_lines=$(cat $TMPF.page-types | wc -l)
	if [ "$nr_lines" -eq 0 ] ; then
        echo "no page with page flag $TARGET_PAGEFLAG found"
        set_return_code FAILED_TO_GET_PFN
	...
	echo "nr_lines: $nr_lines"
	local no_line=$[($RANDOM % $nr_lines) + 1]
    local vaddr="0x$(sed -ne ${no_line}p $TMPF.page-types | cut -f1)"
    local paddr="0x$(sed -ne ${no_line}p $TMPF.page-types | cut -f2)"
	...
	echo "target virtual address: $vaddr"
	echo "target physical address: $paddr"
	__do_multiple_inject $paddr
	return 0
}

control_multiple_injection_race_tmp() {
    local pid="$1"
    local line="$2"

    echo "$line" | tee -a $OFILE
    case "$line" in
        "memeater_random prepared")
			sleep $(echo "scale=4 ; $RANDOM / 100000" | bc)
			$PAGETYPES -p $pid -a 0x700000000,
			__control_multiple_injection_race_process $pid
            kill -SIGUSR1 $pid
            ;;
        "memeater_random exit")
            kill -SIGUSR1 $pid
            set_return_code "EXIT"
            return 0
            ;;
