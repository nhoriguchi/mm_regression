...
int main(int argc, char *argv[]) {
	...
	int *nodes;
	int thp = 0;
	int partialmigrate = 0;

	while ((c = getopt(argc, argv, "vp:m:n:h:tP")) != -1) {
		switch(c) {
                case 'v':
                        verbose = 1;
                        break;
	...
	p = checked_mmap((void *)ADDR_INPUT, nr_hp * HPS, protflag, mapflag, -1, 0);
	if (thp)
		madvise(p, nr_hp * HPS, MADV_HUGEPAGE);
	else
		madvise(p, nr_hp * HPS, MADV_NOHUGEPAGE);
	/* fault in */
